...
function ExpandedReport({ report: rp, onClose }: { report: SystemReport; onClose: () => void }) {
  const c = TIER_CONFIG[rp.tier];
  return <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose} style={{backgroundColor:'rgba(0,0,0,0.7)',backdropFilter:'blur(8px)'}}>
    <div className="relative w-full sm:max-w-lg max-h-[85vh] sm:max-h-[90vh] rounded-t-2xl sm:rounded-sa-lg overflow-hidden" onClick={e=>e.stopPropagation()} style={{border:`1px solid ${c.border}`,backgroundColor:'#151518',boxShadow:rp.tier==='diamond'?'0 0 40px rgba(184,212,232,0.10)':rp.tier==='gold'?'0 0 30px rgba(197,165,90,0.08)':'none'}}>
      {/* Corner frames — behind content, decorative only */}
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden rounded-t-2xl sm:rounded-sa-lg">
        <TierFrame tier={rp.tier}/>
      </div>
      {/* Close button — always visible */}
      <button onClick={onClose} className="absolute top-3 right-3 p-1.5 text-sa-cream-faint hover:text-sa-cream transition-colors rounded-sa-sm hover:bg-sa-bg-lift z-30"><X className="w-4 h-4"/></button>
      {/* Scrollable content — above frames */}
      <div className="overflow-y-auto max-h-[85vh] sm:max-h-[90vh]"><div className="relative z-10 px-5 py-6 sm:p-10">
        <div className="text-center mb-6 sm:mb-8"><p className="text-[0.65rem] uppercase tracking-[0.2em] text-sa-cream-faint mb-3">System Report</p><h2 className="font-serif text-2xl text-sa-cream mb-1">{getMonthLabel(rp.month)}</h2>{rp.isInstallationReport&&<p className="text-xs mt-1" style={{color:c.color}}>Installation Complete — Day 21</p>}<div className="flex justify-center mt-5 mb-3"><ScoreRing score={rp.score} tier={rp.tier} size={80}/></div><div className="flex items-center justify-center gap-3"><span className="text-sm font-medium uppercase tracking-wider" style={{color:c.color}}>{c.label}</span><DeltaBadge delta={rp.scoreDelta}/></div>{rp.scoreCapped&&<div className="flex items-center justify-center gap-1.5 mt-2"><AlertTriangle className="w-3.5 h-3.5 text-sa-cream-faint"/><span className="text-xs text-sa-cream-faint">Score capped — below minimums</span></div>}</div>
        <div className="h-px mb-5" style={{background:`linear-gradient(90deg, transparent, ${c.border}, transparent)`}}/>
        <div className="space-y-3.5 mb-6"><p className="text-[0.65rem] uppercase tracking-[0.15em] text-sa-cream-faint">Performance</p><CategoryBar label={`Habits (${rp.habitsCount})`} score={rp.habitsScore} color={c.color}/><CategoryBar label={`Tasks (${rp.tasksAvgPerDay}/day)`} score={rp.tasksScore} color={c.color}/><CategoryBar label={`NNs (${rp.nnCount})`} score={rp.nnScore} color={c.color}/></div>
        <div className="grid grid-cols-2 gap-2.5 mb-6">{[{icon:CheckCircle,l:'Tasks',v:rp.totalTasksCompleted},{icon:Flame,l:'Streak',v:`${rp.longestStreak}d`},{icon:Calendar,l:'Active',v:`${rp.totalDaysActive}d`},{icon:Target,l:'Score',v:rp.score,useC:true}].map(s=>{const I=s.icon;return <div key={s.l} className="p-2.5 rounded-sa" style={{backgroundColor:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)'}}><div className="flex items-center gap-1.5 mb-1"><I className="w-3.5 h-3.5" style={{color:c.color}}/><span className="text-[0.65rem] uppercase tracking-wider text-sa-cream-faint">{s.l}</span></div><span className="font-serif text-lg" style={{color:(s as any).useC?c.color:'var(--cream)'}}>{String(s.v)}</span></div>;})}</div>
        <div className="text-center"><div className="h-px mb-4" style={{background:`linear-gradient(90deg, transparent, ${c.border}, transparent)`}}/><p className="text-[0.65rem] uppercase tracking-[0.15em] text-sa-cream-faint mb-2.5">Highlight</p><p className="text-sm text-sa-cream-soft italic leading-relaxed">"{rp.personalHighlight}"</p></div>
      </div></div>
    </div>
  </div>;
}
